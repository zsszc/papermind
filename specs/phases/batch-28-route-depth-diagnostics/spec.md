...
- [ ] AC1：纯聚合对深度桶、span、互斥类别和候选映射计算正确且确定性。
- [ ] AC2：畸形/重复 ID、非完整 train、dirty、越界指标和不一致基线 fail closed。
- [ ] AC3：CLI 只读、临时复制向量、离线、无身份/正文输出。
- [ ] AC4：在 clean 提交完成一次真实 train 聚合，冻结唯一下一候选。
- [ ] AC5：全量/公开 Gate、测试报告、台账、分段提交与 push 完成。
