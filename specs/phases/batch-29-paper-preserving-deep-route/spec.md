...
- [x] AC1：纯函数严格保持生产论文 slot 序列与每篇名额，并能用跨路深层命中替换同论文 chunk。
- [x] AC2：不变输入得到与生产完全相同输出；畸形/重复/论文不一致输入 fail closed。
- [x] AC3：共享 RetrievalPipeline 和 eval 仅通过显式 profile 接线，公式指纹写入报告。
- [x] AC4：同提交完整 train 配对 Gate 只输出脱敏聚合，严格执行提升/不回退/延迟边界。
