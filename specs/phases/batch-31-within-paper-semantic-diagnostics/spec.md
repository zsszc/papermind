...
- [x] AC1：完整 train、clean Git、离线环境、私有路径和只读数据契约 fail closed。
- [x] AC2：同一查询向量被全局与论文过滤查询复用，过滤范围只含生产已选论文。
- [x] AC3：分类、覆盖、rank bucket、P50/P95 与固定 Gate 有确定性单测。
- [x] AC4：真实报告只含聚合白名单，并绑定代码/数据/语料/向量/HNSW 指纹。
