...
- [ ] AC1：局部 BM25 只查询生产已选论文，固定词法与排序口径，未选论文无法进入候选。
- [ ] AC2：纯选择保持论文 slot；正分 incumbent 锁定，只替换同论文零分 slot，输入不被修改。
- [ ] AC3：畸形/重复/越界论文、局部查询异常 fail closed；生产 profile 无变化。
- [ ] AC4：eval 报告绑定公式指纹；同提交完整 train 配对 Gate 严格执行全部门槛。
