...
│   ├── tests/              # pytest 套件（1028 用例，内存 SQLite + TestClient）
...
`require('electron')`，确保 CI 无 GUI 也能运行。当前后端 1028 个测试、前端 66 个测试；
...
> 最后更新：2026-09-04，Batch 28 真实 train route-depth 归因完成后同步。
