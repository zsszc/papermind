...
│   ├── tests/              # pytest 套件（1087 用例，内存 SQLite + TestClient）
...
`require('electron')`，确保 CI 无 GUI 也能运行。当前后端 1087 个测试、前端 66 个测试；
...
> 最后更新：2026-09-04，Batch 32 论文内语义候选一次性 dev Gate 完成后同步。
