...
│   ├── tests/              # pytest 套件（1045 用例，内存 SQLite + TestClient）
...
`require('electron')`，确保 CI 无 GUI 也能运行。当前后端 1045 个测试、前端 66 个测试；
...
> 最后更新：2026-09-04，Batch 29 保论文深层候选 train Gate 完成后同步。
