"""Batch 29：保论文集合候选必须经过同提交完整 train 配对 Gate。"""

from __future__ import annotations

import pytest

from eval.paper_preserving_train_gate import evaluate_paper_preserving_train
from eval.run import paper_preserving_deep_route_contract_metadata


def _report(profile: str, *, coverage: float = 0.46) -> dict:
    candidate = profile == "paper-preserving-deep-route-v1"
    ...
    return {
        ...
        "pipeline": {
            ...
            "top_k": 5,
            **({"paper_preserving_deep_route": contract} if candidate else {}),
        },
...
def test_gate_passes_only_with_one_item_coverage_gain_and_no_regression():
    ...
    candidate = _report(
        "paper-preserving-deep-route-v1", coverage=0.45 + 1 / 13
    )

    gate = evaluate_paper_preserving_train(baseline, candidate)
...
def test_gate_fails_closed_for_unpaired_or_incomplete_reports(mutation, match):
    ...
    candidate = _report("paper-preserving-deep-route-v1", coverage=0.55)
    mutation(baseline, candidate)
    with pytest.raises(ValueError, match=match):
        evaluate_paper_preserving_train(baseline, candidate)
