...
锁定 SCHEMA_MIGRATIONS 登记的 7 个迁移分支的现有行为：
...
# 7 个迁移分支的期望默认值（None 表示 ADD COLUMN 不带 DEFAULT，存量行应为 NULL）。
# 新增迁移分支时需同步维护本表。
...
# 旧 schema：三张表都只含迁移前就存在的基础列，缺全部 7 个迁移目标列
_OLD_SCHEMA_DDL = {
...
def _build_old_db(db_path, tables=("papers", "conversations", "messages")):
...
def test_migration_registry_has_exactly_seven_branches():
    """锁定：登记表当前恰好 7 个迁移分支（新增字段登记时应同步更新本测试）。"""
    total = sum(len(cols) for cols in SCHEMA_MIGRATIONS.values())
    assert total == 7
    assert set(SCHEMA_MIGRATIONS) == {"papers", "conversations", "messages"}


class TestSevenMigrationBranches:
    """逐一锁定 7 个 ADD COLUMN 分支：列被补齐且存量行获得预期默认值/NULL。"""
    ...
    def test_all_seven_columns_present_after_migration(self, old_db):
        """锁定：旧库执行一次 ensure_schema 后 7 个登记列全部存在。"""
        ensure_schema()
...
class TestMigrationSemantics:
    ...
    def test_missing_table_silently_skipped(self, tmp_path, monkeypatch):
        ...
        _build_old_db(db_path, tables=("papers", "messages"))  # 无 conversations
        monkeypatch.setattr(database_module, "engine", _make_engine(db_path))
