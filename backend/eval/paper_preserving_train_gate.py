"""Batch 29 保论文集合深层证据候选的完整 train 配对 Gate。"""
...
from eval.run import paper_preserving_deep_route_contract_metadata
...
SCHEMA = "paper-preserving-train-gate-v1"
...
def _validate_report(
    ...
    expected_profile = (
        "paper-preserving-deep-route-v1" if candidate else "hybrid"
    )
    ...
    if candidate:
        contract = paper_preserving_deep_route_contract_metadata()
        formula_sha = _sha(
            benchmark.get("paper_preserving_formula_sha256"),
            "paper_preserving_formula_sha256",
        )
        if formula_sha != contract["formula_sha256"]:
            raise ValueError("候选公式指纹与当前代码不一致")
        if pipeline.get("paper_preserving_deep_route") != contract:
            raise ValueError("候选管线未绑定冻结公式")
    elif (
        "paper_preserving_formula_sha256" in benchmark
        or "paper_preserving_deep_route" in pipeline
    ):
        raise ValueError("基线不得伪装保论文深层候选")
...
def evaluate_paper_preserving_train(
    baseline: dict[str, Any], candidate: dict[str, Any]
) -> dict[str, Any]:
    """验证同提交完整 train 报告，并返回不含逐题身份的晋级判定。"""
    ...
    return {
        ...
        "binding": {
            ...
            **{
            ...
            "paper_preserving_formula_sha256": contract["formula_sha256"],
            "split": "train",
            "top_k": 5,
            "item_count": 13,
        },
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="比较同提交生产基线与保论文深层候选的完整 train 报告"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result = evaluate_paper_preserving_train(baseline, candidate)
        write_report_exclusive(output_path, result)
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        print(f"[paper-preserving-train-gate] FAIL: {type(exc).__name__}")
        return 2
    print(json.dumps({
